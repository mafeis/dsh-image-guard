...
//
// 解决两件真实会锁死会话的事：
//   ① 图片无限累积：agent 的 prompt = 整份对话历史，每看一次图就多留一张；
//      而服务端 --limit-mm-per-prompt.image 是按【整个 prompt】计的，攒够就永久 400，
//      连 compact 都过不去（压缩发的是同一份历史）→ 会话报废。
//   ② 已经超限的会话：从 400 报文里学出服务端上限，自动降级重试，而不是把会话判死。
//
...
function applyInner(ctx, config) {
        ...
        if (trim) {
          ...
          if (st.dryRun) {
            if (attempt === 0 && dropped > 0) {
              ...
              log(`dryRun：本会驱逐 ${dropped} 张（${images.length} → ${images.length - dropped}）`);
            }
          } else {
            payload = r.payload;
            if (attempt === 0 && dropped > 0) {
              // tokensSaved 是**估算**：每张被换掉的图按 tokensPerImage（默认 972 = 该路由实测上限）计，
              ...
              log(`历史图片驱逐：${images.length} → ${images.length - dropped} 张（保留最近 ${keep}），本轮约省 ${saved} token`);
            }
          }
        }
  ...
  flush(true); // 加载即写出状态文件：这样「插件到底有没有生效」一眼可查（不必等第一条带图请求）
  console.error(
    `[image-guard] 已启用：保留最近 ${st.keepRecent} 张图，400 上限自动降级（最多 ${st.maxRetries} 次）` +
      `${st.dryRun ? "（dryRun 只观察不修改）" : ""}；pid=${process.pid}；配置文件 ${file}`,
  );
