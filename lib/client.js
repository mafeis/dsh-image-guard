...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css = [
			...
			".ig-preset{grid-column:2;display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
		...
		var en = {
			...
			presetFill: "Fill in",
			presetZh: "Chinese default",
			presetEn: "English default",
			preview: "Preview",
		...
		// 标记模板的两个语言版本：服务端默认中文（lib/decide.js 的 DEFAULT_MARKER），
		// 参数页提供一键预置，英文语境无需手写占位符。
		var MARKER_ZH = "[图片已省略 #{index}{identity}{hint}]";
		var MARKER_EN = "[image omitted #{index}{identity}{hint}]";

		function previewMarker(tpl, mode) {
			var sel = SAMPLES[mode] || SAMPLES.basename;
			var s = typeof tpl === "string" && tpl ? tpl : MARKER_ZH;
						...
						children: [
							...
							jx.jsx(Field, {
								...
								children: [
									...
									jx.jsx("div", { className: "ig-preset", children: [
										jx.jsx("span", { className: "ig-field-hint", children: t("presetFill") }),
										jx.jsx(Button, { variant: "outline", size: "sm", onClick: function () { g.setPlaceholder(MARKER_ZH); g.dirty(); }, children: t("presetZh") }),
										jx.jsx(Button, { variant: "outline", size: "sm", onClick: function () { g.setPlaceholder(MARKER_EN); g.dirty(); }, children: t("presetEn") }),
									]}),
								],
							}),
						],
