...
//     总览：指标网格（主指标=已省 token）+ 图片保留可视化 + 实现说明 + 关键开关
//     参数：按「开关 / 发送 / 标记」三组分节，全部可调项
//     诊断：链路状态（呼吸圆点）、统计芯片、最近请求形态（斑马纹表格，未拦截时查看）
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css = [
			/* ---------- 骨架 ---------- */
			".ig-root{display:flex;flex-direction:column;gap:12px}",
			".ig-head{display:flex;align-items:center;justify-content:space-between;gap:10px}",
			".ig-title{font-size:16px;font-weight:650;margin:0;color:var(--dsw-alias-label-primary);display:flex;align-items:center;gap:8px}",
			".ig-title::before{content:\"\";width:4px;height:15px;border-radius:2px;background:var(--dsw-alias-state-info-primary,#4c8dff)}",
			...
			/* ---------- 卡片与分组 ---------- */
			".ig-card{border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:12px;padding:14px 16px;background:var(--dsw-alias-bg-layer-2,#1a1e26);display:flex;flex-direction:column;gap:11px;box-shadow:0 1px 3px rgba(0,0,0,.12)}",
			".ig-sect{display:flex;align-items:center;gap:8px;margin:0}",
			".ig-sect::before{content:\"\";width:3px;height:13px;border-radius:2px;background:var(--dsw-alias-state-info-primary,#4c8dff);opacity:.85}",
			".ig-sect b{font-size:12.5px;font-weight:650;color:var(--dsw-alias-label-primary);letter-spacing:.02em}",
			".ig-sect small{font-size:11px;color:var(--dsw-alias-label-tertiary)}",
			/* ---------- 键值行 ---------- */
			".ig-kv{display:grid;grid-template-columns:max-content minmax(0,1fr);gap:7px 14px;font-size:12px;line-height:1.65;margin:0}",
			".ig-kv dt{color:var(--dsw-alias-label-tertiary);margin:0;white-space:nowrap}",
			".ig-kv dd{margin:0;color:var(--dsw-alias-label-secondary);min-width:0;overflow-wrap:anywhere}",
			".ig-kv dd b{color:var(--dsw-alias-label-primary);font-weight:600}",
			/* ---------- 指标网格 ---------- */
			".ig-statgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(96px,1fr));gap:8px}",
			".ig-stat{display:flex;flex-direction:column;gap:2px;padding:9px 11px;border-radius:9px;background:var(--dsw-alias-bg-layer-1,#12151b);border:1px solid var(--dsw-alias-border-l2,#2a2f3a);min-width:0}",
			".ig-stat i{font-style:normal;font-size:11px;color:var(--dsw-alias-label-tertiary);white-space:nowrap}",
			".ig-stat b{font-size:16px;font-weight:650;color:var(--dsw-alias-label-primary);font-variant-numeric:tabular-nums;line-height:1.25;white-space:nowrap}",
			".ig-stat b small{font-size:11px;font-weight:500;color:var(--dsw-alias-label-tertiary);margin-left:2px}",
			".ig-stat.hero{border-color:color-mix(in srgb,var(--dsw-alias-state-info-primary,#4c8dff) 45%,transparent);background:color-mix(in srgb,var(--dsw-alias-state-info-primary,#4c8dff) 8%,var(--dsw-alias-bg-layer-1,#12151b))}",
			".ig-stat.hero b{font-size:19px;color:var(--dsw-alias-state-info-primary,#4c8dff)}",
			/* ---------- 图片保留可视化 ---------- */
			".ig-stripwrap{display:flex;flex-direction:column;gap:6px}",
			".ig-strip{display:flex;align-items:flex-end;gap:3px;flex-wrap:wrap}",
			".ig-sq{width:13px;height:17px;border-radius:3px;background:var(--dsw-alias-bg-layer-1,#12151b);border:1px solid var(--dsw-alias-border-l2,#2a2f3a);opacity:.45;position:relative}",
			".ig-sq.cut::after{content:\"\";position:absolute;left:2px;right:2px;top:50%;height:1px;background:var(--dsw-alias-label-tertiary);transform:rotate(-24deg)}",
			".ig-sq.keep{opacity:1;background:color-mix(in srgb,var(--dsw-alias-state-info-primary,#4c8dff) 26%,var(--dsw-alias-bg-layer-1,#12151b));border-color:color-mix(in srgb,var(--dsw-alias-state-info-primary,#4c8dff) 60%,transparent)}",
			".ig-sqcaption{font-size:11px;color:var(--dsw-alias-label-tertiary);display:flex;gap:12px;flex-wrap:wrap}",
			".ig-sqcaption b{color:var(--dsw-alias-label-secondary);font-weight:600;font-variant-numeric:tabular-nums}",
			/* ---------- 控件 ---------- */
			...
			".ig-num input{width:70px;box-sizing:border-box;background:var(--dsw-alias-bg-layer-1,#12151b);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:6px;padding:4px 6px;font:inherit;font-size:12.5px;font-variant-numeric:tabular-nums;transition:border-color .15s ease}",
			".ig-num input:focus{outline:none;border-color:var(--dsw-alias-state-info-primary,#4c8dff)}",
			...
			/* ---------- 状态与消息 ---------- */
			".ig-dot{display:inline-block;width:8px;height:8px;border-radius:99px;margin-right:6px;vertical-align:middle;background:var(--dsw-alias-label-tertiary)}",
			".ig-dot.ok{background:var(--dsw-alias-state-success-primary,#2ecc71);box-shadow:0 0 0 3px color-mix(in srgb,var(--dsw-alias-state-success-primary,#2ecc71) 18%,transparent)}",
			".ig-dot.err{background:var(--dsw-alias-state-error-primary,#e74c3c);box-shadow:0 0 0 3px color-mix(in srgb,var(--dsw-alias-state-error-primary,#e74c3c) 18%,transparent);animation:ig-pulse 1.6s ease-in-out infinite}",
			"@keyframes ig-pulse{0%,100%{box-shadow:0 0 0 3px color-mix(in srgb,var(--dsw-alias-state-error-primary,#e74c3c) 18%,transparent)}50%{box-shadow:0 0 0 6px color-mix(in srgb,var(--dsw-alias-state-error-primary,#e74c3c) 8%,transparent)}}",
			...
			".ig-dirty{font-size:11px;color:var(--dsw-alias-state-warning-primary,#d9a12e);background:color-mix(in srgb,var(--dsw-alias-state-warning-primary,#d9a12e) 12%,transparent);border-radius:99px;padding:2px 9px}",
			".ig-warn{font-size:12px;line-height:1.55;color:var(--dsw-alias-state-error-primary,#e74c3c);background:color-mix(in srgb,var(--dsw-alias-state-error-primary,#e74c3c) 10%,transparent);border:1px solid color-mix(in srgb,var(--dsw-alias-state-error-primary,#e74c3c) 30%,transparent);border-radius:9px;padding:9px 11px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}",
			/* ---------- 表单 ---------- */
			".ig-field{display:grid;grid-template-columns:max-content minmax(0,1fr);gap:6px 12px;align-items:center;margin:0}",
			...
			".ig-in{width:100%;box-sizing:border-box;background:var(--dsw-alias-bg-layer-1,#12151b);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:7px;padding:5px 8px;font:inherit;font-size:12px;transition:border-color .15s ease}",
			".ig-in:focus{outline:none;border-color:var(--dsw-alias-state-info-primary,#4c8dff)}",
			".ig-mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}",
			/* ---------- 标记预览：模拟模型实际收到的一条消息 ---------- */
			".ig-preview{grid-column:2;display:flex;align-items:flex-start;gap:8px;padding:8px 10px;border-radius:9px;background:var(--dsw-alias-bg-layer-1,#12151b);border:1px dashed var(--dsw-alias-border-l2,#2a2f3a);font-size:11.5px;line-height:1.6;color:var(--dsw-alias-label-secondary);word-break:break-all}",
			".ig-preview::before{content:\"\";flex:none;width:7px;height:7px;border-radius:99px;margin-top:5px;background:var(--dsw-alias-state-info-primary,#4c8dff);opacity:.7}",
			".ig-preview em{font-style:normal;color:var(--dsw-alias-label-tertiary);margin-right:5px;white-space:nowrap}",
			".ig-preview .ig-bubble{min-width:0}",
			/* ---------- 诊断 ---------- */
			".ig-chips{display:flex;gap:6px;flex-wrap:wrap}",
			".ig-chip{display:inline-flex;align-items:baseline;gap:5px;font-size:11px;color:var(--dsw-alias-label-tertiary);background:var(--dsw-alias-bg-layer-1,#12151b);border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:99px;padding:3px 10px;white-space:nowrap}",
			".ig-chip b{font-size:12px;font-weight:600;color:var(--dsw-alias-label-primary);font-variant-numeric:tabular-nums}",
			".ig-diag{display:flex;flex-direction:column;gap:1px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:11px;max-height:240px;overflow:auto;border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:8px}",
			".ig-diag-row{display:grid;grid-template-columns:58px 74px 62px 1fr;gap:8px;padding:4px 9px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;align-items:baseline}",
			".ig-diag-row:nth-child(even){background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#12151b) 55%,transparent)}",
			".ig-diag-head{position:sticky;top:0;background:var(--dsw-alias-bg-layer-2,#1a1e26);color:var(--dsw-alias-label-secondary);font-weight:600;border-bottom:1px solid var(--dsw-alias-border-l2,#2a2f3a)}",
			".ig-diag-row b{color:var(--dsw-alias-label-secondary);font-weight:500}",
			".ig-diag-row .ig-bad{color:var(--dsw-alias-state-error-primary,#e74c3c)}",
			".ig-empty{padding:18px 10px;text-align:center;color:var(--dsw-alias-label-tertiary);font-size:11.5px;display:block;grid-template-columns:none}",
			".ig-hr{border:0;border-top:1px solid var(--dsw-alias-border-l2,#2a2f3a);margin:2px 0}",
			/* ---------- 页脚保存条 ---------- */
			".ig-foot{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:10px 2px 0;border-top:1px solid var(--dsw-alias-border-l2,#2a2f3a)}",
		].join("");
		...
		var en = {
			...
			sTokUnit: "tok",
			stripKept: "sent as-is",
			stripCut: "replaced by markers",
			health: "Chain status",
			...
			hRewrapOwned: "(currently at the chain head)",
			hRewrapBusy: "(reclaiming continuously)",
			hHead: "Chain head",
			hHeadSelf: "this plugin (guard owns the head)",
			hStolen: "Times the head was taken",
			rewrap: "Re-attach",
		...
		function Num(props) {
			return jx.jsx("label", {
				...
				children: [
					...
					jx.jsx("input", { type: "number", min: props.min, max: props.max, value: props.value, onChange: function (e) { props.onChange(e.target.value); } }),
				],
			});
		}
		...
		/** 分组头：4px 竖条 + 标题（+ 可选说明） */
		function Sect(props) {
			return jx.jsx("p", { className: "ig-sect", children: [jx.jsx("b", { children: props.title }), props.hint ? jx.jsx("small", { children: props.hint }) : null] });
		}
		...
		function useGuard(uiLang) {
			...
			var [unsaved, setUnsaved] = React.useState(false);
			...
			function applyValue(v) {
				...
				setMarkerLang(sel);
				// 存的是「某语言的默认模板」= 当初被自动填进去的默认值，不算自定义：按生效语言显示
				var tpl = typeof v.placeholder === "string" ? v.placeholder : "";
				var custom = tpl && tpl !== MARKER_TEMPLATE.zh && tpl !== MARKER_TEMPLATE.en;
				setPlaceholder(custom ? tpl : defaultTemplate(effLang(sel, uiLang)));
				setUnsaved(false);
			}
			...
			return {
				ready: ready, unreachable: unreachable, applied: applied, saveErr: saveErr, unsaved: unsaved, status: status,
				...
				dirty: function () { setApplied(false); setUnsaved(true); },
			};
		}
		...
		function StatusTag(props) {
			...
			var dotTone = g.unreachable ? "err" : g.enabled && !g.dryRun ? "ok" : "";
			return jx.jsx(Tag, { tone: b[1], children: [jx.jsx("i", { className: "ig-dot " + dotTone, "aria-hidden": true }), b[0]] });
		}

		/** 统一保存条：主按钮 + 未保存徽章 + 结果消息 + 可选次级动作 */
		function SaveRow(props) {
			...
			return jx.jsx("div", {
				...
				children: [
					...
					g.unsaved && !g.applied ? jx.jsx("span", { className: "ig-dirty", children: t("unsaved") }) : null,
		...
		/** 图片保留可视化：每个方块 = 会话中的一张历史图片；亮色 = 原样发送，暗色带斜杠 = 替换为标记 */
		function Strip(props) {
			var g = props.g, t = props.t;
			var keepNum = Math.max(0, Math.min(14, isFinite(Number(g.keepRecent)) ? Number(g.keepRecent) : 12));
			var s = g.status || {};
			var total = Math.max(keepNum + 2, Math.min(14, Number(s.maxSeen) || 0), 6);
			var cut = Math.max(0, total - keepNum);
			var sq = function (kept, i) {
				return jx.jsx("span", { className: "ig-sq" + (kept ? " keep" : " cut"), "aria-hidden": true }, "s" + i);
			};
			return jx.jsx("div", {
				className: "ig-stripwrap",
				title: t("keepRecentHint"),
				children: [
					jx.jsx("div", { className: "ig-strip", children: [].concat(
						Array.from({ length: cut }, function (_, i) { return sq(false, i); }),
						Array.from({ length: keepNum }, function (_, i) { return sq(true, cut + i); })
					) }),
					jx.jsx("div", { className: "ig-sqcaption", children: [
						jx.jsx("span", { children: [jx.jsx("b", { children: keepNum }), " " + t("stripKept")] }),
						jx.jsx("span", { children: [jx.jsx("b", { children: cut }), " " + t("stripCut")] }),
					] }),
				],
			});
		}

		/** 指标网格：hero（已省 token）+ 其余小卡 */
		function StatGrid(props) {
			var g = props.g, t = props.t, s = g.status;
			var v = function (x) { return x != null ? x : "—"; };
			var tile = function (key, label, value, unit, title) {
				return jx.jsx("div", { className: "ig-stat" + (key ? " " + key : ""), title: title, children: [jx.jsx("i", { children: label }), jx.jsx("b", { children: [value, unit ? jx.jsx("small", { children: unit }) : null] })] }, label);
			};
			return jx.jsx("div", { className: "ig-statgrid", children: [
				tile("hero", t("sTok"), fmtTokens(s && s.tokensSaved ? s.tokensSaved : 0), t("sTokUnit"), t("sTokTip")),
				tile(null, t("sReq"), v(s && s.chatPosts)),
				tile(null, t("sImg"), v(s && s.bundled)),
				tile(null, t("sTrim"), (s && s.trimmed ? s.trimmed : 0) + "/" + (s && s.imagesDropped ? s.imagesDropped : 0)),
				tile(null, t("sRetry"), s && s.retried != null ? s.retried : 0),
				tile(null, t("sCap"), g.learned != null ? g.learned : "—"),
				tile(null, t("sNow"), g.now),
			] });
		}
			...
			return jx.jsx("div", {
				...
				children: [
					jx.jsx("div", {
						className: "ig-card",
						children: [
							jx.jsx(Sect, { title: t("gStats") }),
							jx.jsx(StatGrid, { g: g, t: t }),
						],
					}),
					...
					jx.jsx("div", {
						className: "ig-card",
						children: [
							jx.jsx(Sect, { title: t("what"), hint: t("how") + " · " + t("effect") }),
						...
						children: [
							...
							jx.jsx("div", {
								...
								children: [
									...
									jx.jsx(Num, { label: t("keepRecent"), value: g.keepRecent, min: 0, max: 50, onChange: function (v) { g.setKeepRecent(v); g.dirty(); } }),
									jx.jsx(Num, { label: t("maxRetries"), value: g.maxRetries, min: 0, max: 10, onChange: function (v) { g.setMaxRetries(v); g.dirty(); } }),
								],
							}),
						...
						children: [
							...
							jx.jsx(Field, {
								...
								children: [
									...
									jx.jsx("div", { className: "ig-preview", children: [jx.jsx("em", { children: t("preview") }), jx.jsx("span", { className: "ig-bubble", children: previewMarker(g.placeholder, g.pathMode, g.effLang) })] }),
								],
							}),
							jx.jsx(Field, {
								label: t("matchPath"),
							...
							jx.jsx(SaveRow, {
								g: g, t: t,
								extra: [
									jx.jsx(Button, { key: "reload", variant: "outline", size: "sm", onClick: g.reloadNow, children: t("reload") }),
									jx.jsx(Button, { key: "reset", variant: "outline", size: "sm", disabled: g.unreachable, onClick: g.resetNow, children: t("reset") }),
								],
							}),
						],
					}),
		...
		function DiagTab(props) {
			...
			var chip = function (key, label, value) {
				return jx.jsx("span", { className: "ig-chip", children: [label + " ", jx.jsx("b", { children: value })] }, key);
			};
			return jx.jsx("div", {
				className: "ig-root",
				children: [
					jx.jsx("div", {
						className: "ig-card",
						children: [
							jx.jsx(Sect, { title: t("health") }),
							jx.jsx("dl", {
								className: "ig-kv",
								children: [].concat(
									kv("health", s.fetchOwned === false ? jx.jsx("b", { children: [jx.jsx("i", { className: "ig-dot err", "aria-hidden": true }), t("hOwnedNo")] }) : s.fetchOwned === true ? jx.jsx("b", { children: [jx.jsx("i", { className: "ig-dot ok", "aria-hidden": true }), t("hOwned")] }) : "—"),
									...
									kv("hRewrap", String(s.rewraps != null ? s.rewraps : 0) + (s.fetchOwned === true ? " " + t("hRewrapOwned") : s.fetchOwned === false ? " " + t("hRewrapBusy") : "")),
									kv("hHead", s.fetchOwned === true ? t("hHeadSelf") : String(s.headName || "—")),
									kv("hStolen", String(s.stolenSeen != null ? s.stolenSeen : 0)),
								),
							}),
							jx.jsx("div", { className: "ig-chips", children: [
								chip("uptime", t("uptime"), s.startedAt ? fmtUptime(s.startedAt) : "—"),
								chip("cap", t("cap"), g.learned != null ? String(g.learned) : "—"),
								chip("maxSeen", t("maxSeen"), String(s.maxSeen != null ? s.maxSeen : 0)),
								chip("drySkips", t("drySkips"), String(s.dryRunSkips != null ? s.dryRunSkips : 0)),
								chip("sTok", t("sTok"), fmtTokens(s.tokensSaved || 0) + " " + t("sTokUnit")),
							] }),
					...
					jx.jsx("div", {
						className: "ig-card",
						children: [
							jx.jsx(Sect, { title: t("diag") }),
							jx.jsx("div", {
								className: "ig-diag",
								children:
									diag.length === 0
										? jx.jsx("div", { className: "ig-diag-row ig-empty", children: t("diagEmpty") })
										: [head].concat(
													...
													return jx.jsx(
														"div",
														{
															...
															children: [
																...
																jx.jsx("span", { className: d.read === false ? "ig-bad" : undefined, children: d.read === false ? t("readFalse") : "imgs=" + (d.imgs != null ? d.imgs : "?") + " msgs=" + (d.msgs != null ? d.msgs : "?") + " bytes=" + (d.bytes != null ? d.bytes : "?") }),
															],
														},
														"d" + i,
													);
