...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			placeholder: "省略标记模板",
			placeholderHint: "可用占位符 {index} {name} {path} {id} {mime} {dims} {size} {identity} {hint}；{total}/{kept} 会导致前缀缓存失效。",
			preview: "预览",
			pmFull: "完整绝对路径",
			pmRelative: "相对工作区（或 ~）",
			pmBasename: "仅文件名（推荐）",
		...
		var en = {
			...
			placeholderHint: "Placeholders: {index} {name} {path} {id} {mime} {dims} {size} {identity} {hint}; {total}/{kept} invalidate the prefix cache.",
			preview: "Preview",
