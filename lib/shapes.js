...
import { defaultMarker } from "./decide.js";
 ...
 */
/**
 * 标记正文里由本插件生成的说明文字。模板只决定外壳，这几句必须跟着 markerLang 走——
 * 否则英文界面里标题是英文、正文仍是中文（模板换不掉 hint/identity）。
 */
export const MARKER_TEXT = {
	zh: {
		...
		hintReadImage: " · 需要时用 read_image 重新读取",
		hintPathHidden: " · 路径已按配置隐去",
		hintRevisit: " · 需要时可重新访问该地址",
		hintUnrecoverable: " · 无原文件路径，已无法取回",
	},
	en: {
		imageLabel: "image ",
		inlineLabel: "inline image ",
		hintSearch: " · search the workspace by file name to read it again",
		hintReadImage: " · read it again with read_image",
		hintPathHidden: " · path hidden by configuration",
		hintRevisit: " · revisit the URL to view it",
		hintUnrecoverable: " · no source path, cannot be retrieved",
	},
};

/** 取某个语言的标记文字表；除 en 外一律中文。 */
export const markerText = (lang) => (lang === "en" ? MARKER_TEXT.en : MARKER_TEXT.zh);
...
export function stripImages(body, keep, template, opts = {}) {
  ...
  for (let i = 0; i < dropped; i++) {
    ...
    const info = extractImageInfo(parent[pi], parent, pi, {
      ...
      markerLang: opts.markerLang,
    });
    parent[pi] = { type: "text", text: renderMarker(info, template, opts.markerLang) };
  }
...
export function extractImageInfo(part, content, pi, ctx = {}) {
  ...
  const L = markerText(ctx.markerLang);
  const fullPath = path;
  if (mode === "none") {
    path = "";
    name = "";
  ...
  else label = (name ? name + " · " : "") + (mode === "none" && fullPath ? L.imageLabel : L.inlineLabel) + (id || "?") + (inline ? " (" + inline + ")" : "");
...
export function renderMarker(info, template, lang) {
  const tpl = typeof template === "string" && template ? template : defaultMarker(lang);
  const map = {
