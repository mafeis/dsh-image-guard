 ...
 * （DSH 自己的 `contentHasImage` 就是递归走的），只扫一层会**既数不到也裁不掉**——
 * 症状正好是"看着在跑却拦不住 400"。所以这里递归遍历，路径形如
...
/** 深拷贝并把除「最近 keep 张」以外的图片换成**带身份信息的标记**（不改原对象）。
 *
 * 关键点：标记必须让模型/agent 知道「少了哪张、去哪找」。
 * DSH 的图片在消息里是成对出现的：
...
/** 用模板渲染标记。模板不含 `{...}` 时原样返回（兼容把标记写死的老配置）。 */
