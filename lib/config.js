...
import { DEFAULT_MARKER, DEFAULT_MARKER_EN } from "./decide.js";
...
export function normalize(raw) {
  ...
  return {
    ...
    matchPath,
    // 等于某语言默认模板的值不算自定义，折叠成空（用生效语言的默认模板）
    placeholder: typeof m.placeholder === "string" && !isDefaultMarker(m.placeholder) ? m.placeholder : d.placeholder,
...
/** 写的正好是某语言的默认模板：等同于「没自定义」，跟着生效语言走。
 *  （0.8.11 及更早的设置页会把默认模板当普通值存进配置，不回退就会在英文界面里留一段中文。） */
export function isDefaultMarker(text) {
  return text === DEFAULT_MARKER || text === DEFAULT_MARKER_EN;
}
