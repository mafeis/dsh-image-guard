...
**文档、表述与打包整理**（合并记录 0.8.3–0.8.12 期间的整理工作）
...
- 打包与规范：新增 `.github/workflows/test.yml`（无需安装依赖即可运行 72 项测试）与 `CONTRIBUTING.md`；移除无对应依赖的 `lint` 脚本；`@deepseek-ai/schemastery` 的 peer 范围加入预发布分支；市场条目补充 Release tarball 地址。
...
- 新增 `markerLang`（`zh` / `en`）：标记语言同时作用于**默认模板**与标记正文（身份标签、取回提示），因此 `en` 下整条标记不含中文；`placeholder` 留空即取该语言的默认模板。设置页「参数」新增**标记语言**下拉，切换时会把仍是默认值的模板一起换掉、自定义模板保持不动，并修正了此前英文界面里模板框与预览仍显示中文的问题。
- 删除测试中两个互调但无人使用的辅助函数。
