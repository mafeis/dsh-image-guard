...
**文档、表述与打包整理**（合并记录 0.8.3–0.8.13 期间的整理工作）
...
- 打包与规范：新增 `.github/workflows/test.yml`（无需安装依赖即可运行 78 项测试）与 `CONTRIBUTING.md`；移除无对应依赖的 `lint` 脚本；`@deepseek-ai/schemastery` 的 peer 范围加入预发布分支；市场条目补充 Release tarball 地址。
